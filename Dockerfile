...
RUN BUILD_DEPS="curl ca-certificates xz-utils" && \
    ...
    cd /tmp && \
    curl -k -SLO https://github.com/PostgREST/postgrest/releases/download/${POSTGREST_VERSION}/postgrest-${POSTGREST_VERSION}-ubuntu.tar.xz && \
...
COPY setup-script.sql /docker-entrypoint-initdb.d/setup-script.sql
